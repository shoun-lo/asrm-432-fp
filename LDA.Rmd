...
nb_model <- train(x = train_data[, -86], y = as.factor(train_data$X86), method = "nb", trControl = ctrl)
print(nb_model)
```
```{r}
...
heatmap(correlation_matrix, 
        ...
        main = "Heatmap of train_data Correlation Matrix")
```
...
training_lda <- lda(train_data$X86 ~ ., data = train_data)
fitted_lda <- predict(training_lda, data = train_data)

conf_matrix <- table(fitted_lda$class, train_data$X86)
conf_matrix
```
```{r}
insample_accuracy <- (table(fitted_lda$class, train_data$X86)[1,1] + table(fitted_lda$class, train_data$X86)[2,2]) / dim(train_data)[1]
insample_accuracy
...
training_lda
```
**Comment** The LDA model fits the training set really well which could mean that the best decision boundary is a linear one and the matrix is best left common and not class specific, here I will try QDA to check if this is true
```{r}
library(MASS)

training_qda <- qda(train_data$X86 ~ ., data = train_data)
fitted_qda <- predict(training_qda, data = train_data)
table(fitted_qda$class, train_data$X86)
```
...
```{r}
lm_data <- lm(train_data$X86 ~ ., data = train_data)
...
```{r}
#train_optimal <- lm(train_data$X86 ~ X1 + X4 + X5 + X6 + X10 + X17 + X18 + X21 + 
    #X22 + X28 + X30 + X35 + X36 + X41 + X42 + X43 + X44 + X46 + 
    #X47 + X55 + X57 + X58 + X59 + X63 + X76 + X78 + X79 + X80 + 
    #X82 + X83 + X84 + X85, data = train_data)

lda_train_opt <- lda(train_data$X86 ~ X1 + X4 + X5 + X6 + X10 + X17 + X18 + X21 + 
    X22 + X28 + X30 + X35 + X36 + X41 + X42 + X43 + X44 + X46 + 
    X47 + X55 + X57 + X58 + X59 + X63 + X76 + X78 + X79 + X80 + 
    X82 + X83 + X84 + X85, data = train_data)
fitted_opt <- predict(lda_train_opt, data = train_data)

conf_matrix <- table(fitted_opt$class, train_data$X86)
conf_matrix

insample_accuracy <- (table(fitted_opt$class, train_data$X86)[1,1] + table(fitted_opt$class, train_data$X86)[2,2]) / dim(train_data)[1]
insample_accuracy
...
test_lda <- lda(test_data$X86 ~ ., data = test_data)
fitted_lda <- predict(test_lda, data = test_data)

table(fitted_lda$class, test_data$X86)
```
